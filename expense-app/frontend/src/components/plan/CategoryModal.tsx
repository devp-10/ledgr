...
import { BudgetCategory, BudgetGroup, CategorizationRule } from '../../types'
...
const MATCH_TYPES: CategorizationRule['matchType'][] = ['contains', 'starts_with', 'ends_with', 'regex']

export function CategoryModal({ category, groups, open, onClose, onSave, onDelete }: CategoryModalProps) {
  const [name, setName] = useState(category?.name ?? '')
  const [budget, setBudget] = useState(String(category?.budgetAmount ?? 0))
  const [group, setGroup] = useState(category?.group ?? groups[0]?.id ?? '')
  const [emoji, setEmoji] = useState(category?.emoji ?? '')
  const [rules, setRules] = useState<CategorizationRule[]>(category?.rules ?? [])
  const [newRule, setNewRule] = useState({ matchType: 'contains' as CategorizationRule['matchType'], pattern: '' })
  ...
  const testMatch = () => {
    ...
    try {
      if (newRule.matchType === 'contains') return testInput.toLowerCase().includes(newRule.pattern.toLowerCase())
      if (newRule.matchType === 'starts_with') return testInput.toLowerCase().startsWith(newRule.pattern.toLowerCase())
      if (newRule.matchType === 'ends_with') return testInput.toLowerCase().endsWith(newRule.pattern.toLowerCase())
      if (newRule.matchType === 'regex') return new RegExp(newRule.pattern, 'i').test(testInput)
    } catch { return false }
    return false
  }
  ...
  const handleSave = () => {
    onSave(category.id, {
      name: name.trim(),
      budgetAmount: parseFloat(budget.replace(/[^0-9.]/g, '')) || 0,
      group,
      emoji: emoji.trim(),
      rules,
    })
  ...
  const addRule = () => {
    ...
    setRules(prev => [...prev, { id: Date.now().toString(), ...newRule, categoryId: category.id }])
    setNewRule({ matchType: 'contains', pattern: '' })
  }
          ...
          <div className="space-y-2 mb-3">
            {rules.map(rule => (
              <div key={rule.id} className="flex items-center gap-2 p-2.5 rounded-lg bg-gray-50 dark:bg-gray-700/50 text-sm">
                <span className="text-gray-500 dark:text-gray-400 text-xs">{rule.matchType}</span>
          ...
          <div className="space-y-2.5">
            <div className="flex gap-2">
              <select
                ...
                onChange={e => setNewRule(r => ({ ...r, matchType: e.target.value as CategorizationRule['matchType'] }))}
